...
namespace MCGalaxy {
    ...
    public sealed class PluginTempBot : Plugin {
        ...
        public static ConcurrentDictionary<string, Tinfo> tinfoFor = new ConcurrentDictionary<string, Tinfo>();
        ...
        public override void Unload(bool shutdown) {
            ...
            //Make sure to free up any used IDs when unloading the plugin
        ...
        static bool ClickOnBot(Player p, byte id) {
            foreach (PlayerBot b in tinfoFor[p.name].botList) {
                ...
                byte botID;
                if (!p.EntityList.GetID(b, out botID)) continue;
        ...
        static void DespawnAll(Player p) {
            if (!tinfoFor.ContainsKey(p.name)) return;
            Tinfo t = tinfoFor[p.name];
            foreach (PlayerBot bot in t.botList) {
        ...
        public sealed class CmdTempBot : Command2 {
            ...
            void Add(Player p, string botName, float x, float y, float z, int yaw, int pitch, string skin, string botNick = "empty") {
                ...
                foreach (PlayerBot b in tInfo.botList) {
                ...
                tInfo.botList.Add(bot);
                tInfo.modelList.Add(bot);
            ...
            public static void Remove(Player p, PlayerBot bot) {
                if (bot != null) {
                    ...
                    Tinfo tInfo = tinfoFor[p.name];
                    tInfo.botList.Remove(bot);
                    tInfo.modelList.Remove(bot);
                }
            }
            ...
            public static void RotAxisBot(Player p, PlayerBot bot, EntityProp axis, int angle) {
                ...
                if (p.Supports(CpeExt.EntityProperty)) {
                    ...
                    p.EntityList.SendProp(bot, axis, angle);
                }
            }
            ...
            public static void RotBot(Player p, PlayerBot bot, int rotx, int roty, int rotz) {
                ...
                if (p.Supports(CpeExt.EntityProperty)) {
                    ...
                    p.EntityList.SendProp(bot, EntityProp.RotX, rotx);
                    p.EntityList.SendProp(bot, EntityProp.RotY, roty);
                    p.EntityList.SendProp(bot, EntityProp.RotZ, rotz);
                }
            }
            ...
            public static void ScaleAxisBot(Player p, PlayerBot bot, EntityProp axis, float scale) {
                ...
                if (p.Supports(CpeExt.EntityProperty)) {
                    ...
                    p.EntityList.SendProp(bot, axis, packed);
                }
            }
            ...
            public static void ScaleBot(Player p, PlayerBot bot, float scalex, float scaley, float scalez) {
                ...
                if (p.Supports(CpeExt.EntityProperty)) {
                    ...
                    p.EntityList.SendProp(bot, EntityProp.ScaleX, packedx);
                    p.EntityList.SendProp(bot, EntityProp.ScaleY, packedy);
                    p.EntityList.SendProp(bot, EntityProp.ScaleZ, packedz);
                }
            }
        ...
        public class Tinfo {
            ...
            static void GetTempbotModelList(Player p, out List<PlayerBot> modelList) {
                //For CustomModels support
                const string KEY = "TempBot_BotList";
                object o;
                if (p.Extras.TryGet(KEY, out o)) {
                    modelList = (List<PlayerBot>)o;
                } else {
                    modelList = new List<PlayerBot>();
                    p.Extras[KEY] = modelList;
                }
            }
            ...
            public Tinfo(Player p) {
                ...
                GetTempbotModelList(p, out modelList);
                //TODO rethink this, make it a static list for multiple plugins
                //MsgDev("TEMPBOT: {0} new p extras botlist", p.name);
            }
            public Player p;
            public List<PlayerBot> botList = new List<PlayerBot>();
            public List<PlayerBot> modelList; //For CustomModels support. We need to have all our bots in this list.
            ...
            static string Battery(float percent) {
                ...
                if (percent <= 0.25f) { healthColor = "&c"; } else if (percent <= 0.5f) { healthColor = "&e"; }
